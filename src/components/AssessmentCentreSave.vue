          ...
          <v-container>
            <v-layout row wrap>
              <v-flex xs12>
                <div class="iframe-container">
                  <iframe id="iframe" ref="iframe" style="border: none; width: 100%;" :src="eaUrl" scrolling="no"></iframe>
                </div>
              </v-flex>
            </v-layout>
            <!--
            <v-layout row wrap v-if="ac.role !== 'ac' && !isGuest">
              ...
              <v-flex xs12 sm10 offset-sm1 class="text-xs-right">
                <v-btn v-if="ac.role === 'na'" small color="info" @click="dlgWorkingPlan = true"><icon scale="0.9" class="fa" name="clock"></icon> Working plan</v-btn>
                <v-btn v-if="ac.role === 'na' || ac.role === 'ac'" small color="info" @click="dlgViewCalendar = true"><icon scale="0.9" class="fa" name="calendar-alt"></icon> Display events</v-btn>
                <v-btn v-if="ac.role === 'na'" small color="info" @click="dlgSetUnavPeriod = true"><icon class="fa" name="user-slash"></icon> Set unavailable period</v-btn>
                <v-btn small color="error" @click="cancelRegistrationDlg = true"><icon class="fa" name="user-times"></icon> Cancel Registration</v-btn>
              </v-flex>

              <v-dialog v-if="ac.role === 'na' || ac.role === 'ac'" v-model="dlgWorkingPlan" fullscreen hide-overlay persistent>
                <v-card>
                  <v-card-title class="headline grey lighten-2">
                    Working Plan
                    <v-spacer></v-spacer>
                    <a @click="dlgWorkingPlan = false"><icon name="times" class="fa"></icon></a>
                  </v-card-title>
                  <v-container>
                    <v-layout row wrap>
                      <v-flex xs12>
              ...
              <v-dialog v-if="ac.role === 'na' || ac.role === 'ac'" v-model="dlgViewCalendar" fullscreen hide-overlay persistent>
                <v-card>
                  <v-card-title class="headline grey lighten-2">
                    Events
                    <v-spacer></v-spacer>
                    <a @click="dlgViewCalendar = false"><icon name="times" class="fa"></icon></a>
                  </v-card-title>
                  <v-container>
                    <v-layout row wrap>
                      <v-flex xs12>
                        <full-calendar @view-render="test($event)" @day-click="test($event)" @event-selected="test($event)" :events="appointments"></full-calendar>
                      </v-flex>
                    </v-layout>
                    <v-layout row wrap mt-3>
                      <v-spacer></v-spacer>
                      <v-btn small color="error" @click="dlgViewCalendar = false"><icon class="fa" name="times"></icon> Close</v-btn>
                    </v-layout>
                  </v-container>
                </v-card>
              </v-dialog>

              <v-dialog v-if="ac.role === 'na'" width="500" v-model="dlgSetUnavPeriod" persistent>
                <v-card>
                  <v-card-title class="headline grey lighten-2">
                    Unavailable period
                  </v-card-title>
                  <v-container>
                    ...
                    <v-layout row wrap>
                      ...
                      <v-btn small color="error" @click="dlgSetUnavPeriod = false"><icon class="fa" name="times"></icon> Cancel</v-btn>
                    </v-layout>
                  </v-container>
                </v-card>
              </v-dialog>
...
export default {
  ...
  methods: {
    ...
    resizeIframe() {
      ...
      //this.$refs.iframe.height = this.$refs.iframe.contentWindow.document.body.scrollHeight + 'px';

      //var iframes = iFrameResize({log:true}, '#iframe');
      //this.$refs.iframe.iFrameResize([{}]);
    },
            ...
            if (event.data.result.response.code === "success") {
              ...
              this.eaUrl =
                ...
                (this.acRole === "student"
                  ? "appointments/index/"
                  : "backend/index/");
              this.eaUrl += `?ac=${this.ac.settings.token}&jwt=${

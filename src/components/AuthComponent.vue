      ...
      <v-flex md10>
        ...
        <v-card>
          <v-tabs v-model="activeTab" centered color="transparent" icons-and-text>
            ...
            <v-tab @click="redirect('login')" href="#tab-login">Log in
              <v-icon>verified_user</v-icon>
            </v-tab>
            <v-tab v-if="target !== 'public'" @click="redirect('signup')" href="#tab-signup">Sign up
              <v-icon>account_circle</v-icon>
            </v-tab>
            <v-tab @click="redirect('reset-password')" href="#tab-reset-password">Reset password
              <v-icon>email</v-icon>
            </v-tab>
                  ...
                  <v-card-actions class="pb-5">
                    ...
                    <v-tooltip bottom :color="signupValidationColor">
                      ...
                      >
                        ...
                        <icon v-if="loading" name="circle-notch" spin style="color: gray;"></icon>&nbsp;Sign up
                      </v-btn>
                      <span>{{signupValidationMessage}}</span>
                    </v-tooltip>
                    <v-btn
                      @click="resetSignupForm()"
                      class="white--text warning ml-2"
                      slot="activator"
                    >
...
export default {
  ...
  methods: {
    redirect(action) {
      this.$router.push(`${this.$store.state.homeUrl}/${action}`);
    },
